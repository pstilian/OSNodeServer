      ...
      /* Create child process */
        ...
        if (pid == 0) {
         /* This is the client process */
            ...
            for(eye = 0; eye<5; eye++) printf("%d ", playerInfo->sockArray[eye]);
            printf("\n");
            close(newsockfd);
...
void rungame(int sock, int id){
        ...
        bzero(buffer, 256);
        buffer[150] = playerInfo->gameNum;
        buffer[0] = 1;
        if(playerInfo->gameBoard[i][j]!=gameBoardMap[i][j]){
            buffer[1] = scoreMap[i][j]; //add score to buffer
            playerInfo->scores[id] +=scoreMap[i][j]; //increase player score
        }
        else{
        ...
        int idx;
        printf("----Game %d-----\nCurrent Scores:\nPID\t|Score\n", playerInfo->gameNum);
        for(idx = 0; idx<5;idx++) printf("%d\t|%d\n", playerInfo->sockArray[idx], playerInfo->scores[idx]);
        printf("\n");
